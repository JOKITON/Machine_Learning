...
def setup_layers(f_actv, f_der_actv, layer_shape, seed):
    layers = []
    for layer in range(N_LAYERS):
        prev_neurons = layer_shape[layer][0]
        out_neurons = layer_shape[layer][1]
        if (layer != (N_LAYERS - 1)):
            layers.append(DenseLayer(prev_neurons, out_neurons, leaky_relu, der_leaky_relu, seed))
        else:
            layers.append(DenseLayer(prev_neurons, out_neurons, f_actv, f_der_actv, seed))
